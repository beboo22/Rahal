...
{
    internal class HotelPaymentCommandHandler: ICommandHandler<HotleCreatePayment, ApiResponse>
    {
        private IWriteGenericRepo<PaymentRequest> _Wrepo;
        private IReadGenericRepo<PayHotel> _ROrepo;
        private IWriteUnitOfWork _unitOfWork;

        private IPaymobService paymobService;

        public HotelPaymentCommandHandler(IPaymobService paymobService, IReadGenericRepo<PayHotel> rOrepo, IWriteGenericRepo<PaymentRequest> wrepo, IWriteUnitOfWork unitOfWork)
        {
        ...
        public async Task<ApiResponse> Handle(HotleCreatePayment request, CancellationToken cancellationToken)
        {

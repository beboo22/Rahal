...
{
    internal class FlightPaymentCommandHandler: ICommandHandler<FlightCreatePayment, ApiResponse>
    {
        private IWriteGenericRepo<PaymentRequest> _Wrepo;
        private IReadGenericRepo<PayFlight> _ROrepo;
        private IWriteUnitOfWork _unitOfWork;

        private IPaymobService paymobService;

        public FlightPaymentCommandHandler(IPaymobService paymobService, IReadGenericRepo<PayFlight> rOrepo, IWriteGenericRepo<PaymentRequest> wrepo, IWriteUnitOfWork unitOfWork)
        {
        ...
        public async Task<ApiResponse> Handle(FlightCreatePayment request, CancellationToken cancellationToken)
        {
